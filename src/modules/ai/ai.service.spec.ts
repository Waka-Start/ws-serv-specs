...
describe("AiService", () => {
  ...
  beforeEach(async () => {
    ...
    mockPrismaService.wakaSpecAiJob.create.mockResolvedValue({
      wid: "job-wid-1",
    });
  ...
  describe("ventilate", () => {
    ...
    it("should throw ConflictException when a VENTILATE job is already running", async () => {
      ...
      prisma.wakaSpecAiJob.findFirst.mockResolvedValueOnce({
        wid: "existing-job",
      });
      ...
      await expect(
        ...
        expect.objectContaining({
          message: expect.stringContaining("VENTILATE job"),
        }),
      );
    });
  ...
  describe("ventilateSubChapters", () => {
    it("should create a VENTILATE_SUBCHAPTERS job and return jobId", async () => {
      ...
      prisma.wakaSpecAiJob.create.mockResolvedValueOnce({
        wid: "sub-job-wid-1",
      });
  ...
  describe("deleteContent", () => {
    ...
    it("should preserve remaining content when text not found", async () => {
      ...
      prisma.wakaSpecification.findUnique.mockResolvedValueOnce({
        id: 1,
        wid: "spec-1",
        chapters: [{ id: 10, chapterWid: "ch-1", content: originalContent }],
      });
      prisma.wakaSpecChapterContent.update.mockResolvedValueOnce({
        id: 10,
    ...
    it("should log deletion in AI history with null aiResponse", async () => {
      prisma.wakaSpecification.findUnique.mockResolvedValueOnce({
        id: 1,
        wid: "spec-1",
        chapters: [{ id: 10, chapterWid: "ch-1", content: "Some content" }],
      });
  ...
  describe("suggestQuestions", () => {
    ...
    it("should return questions from tool_use block", async () => {
      const questions = [
        ...
        {
          question: "Combien d’utilisateurs simultanement ?",
          category: "contrainte",
        },
      ];
