...
export class AiController {
  ...
  @ApiOperation({
    summary:
      "Ventiler un texte initial dans tous les chapitres (async — retourne un jobWid)",
  })
  @ApiResponse({
    status: 202,
    description: "Job créé — polling via GET /api/ai/jobs/:jobWid",
  })
  @ApiResponse({
    status: 409,
    description: "Un job VENTILATE est déjà en cours pour cette specification",
  })
  ventilate(
  ...
  @ApiOperation({
    summary:
      "Ventiler le contenu d un chapitre dans ses sous-chapitres template (async — retourne un jobId)",
  })
  @ApiParam({
    name: "chapterWid",
    description: "WID du chapitre template a ventiler",
  })
  @ApiResponse({
    status: 202,
    type: VentilateSubChaptersResponseDto,
    description: "Job cree — polling via GET /api/ai/jobs/:jobId",
  })
  @ApiResponse({
    status: 404,
    description: "Specification ou chapitre non trouve",
  })
  @ApiResponse({
    status: 409,
    description: "Chapitre vide ou job deja en cours",
  })
